...
The voice assistant supports user interruption while the AI is speaking.

During AI Text-to-Speech playback, a separate microphone monitor detects when the user starts speaking. When an interruption is detected:

\* The current AI audio playback is stopped.

\* The microphone monitor is stopped.

\* The user's new speech is recorded.

\* The speech is converted to text using Deepgram.

\* The new message is sent to the AI conversation system.

\* The AI generates an updated response.

\* The updated response is converted to speech and played back.

This allows the user to naturally interrupt the AI instead of waiting for the complete AI response to finish.

**### Voice Conversation Pause / Resume**

The voice conversation can be paused manually using the microphone button.

When paused:

\* Active recording is stopped.

\* AI audio playback is stopped if it is currently playing.

\* Automatic listening/restart is disabled.

\* The voice status message is cleared.

Clicking the microphone button again resumes the voice conversation.
...
\* Voice interruption / barge-in detection while AI is speaking

\* Automatic stopping of AI voice playback when the user interrupts

\* Automatic processing of the interrupted user message

\* Continuous voice conversation after an interruption

\* Manual pause/resume for voice conversations
...
This project implements a configurable small-business missed-call assistant using a generic workflow system, AI conversation simulator, text and voice-based interaction, voice interruption/barge-in handling, manual voice pause/resume, customer records, Speech-to-Text, Text-to-Speech, and real Google Calendar agent tool calling.